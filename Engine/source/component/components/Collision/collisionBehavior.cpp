...
IMPLEMENT_CALLBACK( CollisionBehaviorInstance, onCollision, void, ( ComponentInstance* obj, SceneObject *collObj, VectorF vec, F32 len ), ( obj, collObj, vec, len ),
...
IMPLEMENT_CALLBACK( CollisionBehaviorInstance, onRaycastCollision, void, ( ComponentInstance* obj, SceneObject *collObj, Point3F pos, Point3F normal ), ( obj, collObj, pos, normal ),
                   "@brief Called when we collide with another object.\n\n"
                   "@param obj The ShapeBase object\n"
                   "@param collObj The object we collided with\n"
                   "@param vec Collision impact vector\n"
...
IMPLEMENT_CALLBACK( CollisionBehaviorInstance, onContact, void, ( ComponentInstance* obj, SceneObject *collObj ), ( obj, collObj ),
                   "@brief Called when we collide with another object.\n\n"
                   "@param obj The ShapeBase object\n"
                   "@param collObj The object we collided with\n"
                   "@param vec Collision impact vector\n"
...
{
   addComponentField("blockColldingObject", "If enabled, will stop colliding objects from passing through.", "bool", "1", "");

   addComponentField("collisionMasks", "Lets us define what masks to collide against.", "collisionMask", "-1", "");
...
//////////////////////////////////////////////////////////////////////////
ComponentInstance *CollisionBehavior::createInstance()
{
...
//==========================================================================================
CollisionBehaviorInstance::CollisionBehaviorInstance( Component *btemplate ) 
{
   mTemplate = btemplate;
   mOwner = NULL;
...
bool CollisionBehaviorInstance::onAdd()
{
   ...
   //mOwner->registerCachedInterface("collision", "checkCollisions", this, &mColInterface );
...
void CollisionBehaviorInstance::onComponentAdd()
{
   Parent::onComponentAdd();

   //mOwner->registerCachedInterface( "collision", "checkCollisions", this, &mCollisionInterface );
}

void CollisionBehaviorInstance::onComponentRemove()
{
...
CollisionState *state = mConvexList->findClosestState(mat, mOwner->getScale(), scaledColTol);
if (state && state->dist <= scaledColTol) 
mConvexList->getCollisionInfo(mat, mOwner->getScale(), &mCollisionList, scaledColTol);
...
return mOwner->getTransform().getPosition() + *velocity * travelTime;
...
scaledBox.minExtents.convolve( mOwner->getScale() );
scaledBox.maxExtents.convolve( mOwner->getScale() );
...
bool CollisionBehaviorInstance::checkEarlyOut(Point3F start, VectorF velocity, F32 time)
{
   ...
   if (mFabs(distance.x) < mOwner->getObjBox().len_x() &&
      mFabs(distance.y) < mOwner->getObjBox().len_y() &&
      mFabs(distance.z) < mOwner->getObjBox().len_z())
   {
      // We can potentially early out of this.  If there are no polys in the clipped polylist at our
      //  end position, then we can bail, and just set start = end;
      Box3F wBox = mConvexList->getBoundingBox(mOwner->getTransform(), mOwner->getScale());  
...
//wBox.minExtents = pos + mOwner->getObjBox()/*mScaledBox*///.minExtents - exp;
//wBox.maxExtents.x = pos.x + mOwner->getObjBox()/*mScaledBox*/.maxExtents.x;
//wBox.maxExtents.y = pos.y + mOwner->getObjBox()/*mScaledBox*/.maxExtents.y;
//wBox.maxExtents.z = pos.z + mOwner->getObjBox()/*mScaledBox*/.minExtents.z + sTractionDistance;
...
wBox.maxExtents.z = pos.z + mOwner->getObjBox()/*mScaledBox*///.maxExtents.z;
...
if ( mOwner->getWorldBox().isOverlapped( obj->getWorldBox() ) )
{
...
if (  mOwner->getWorldBox().isOverlapped(item->getWorldBox()) &&
(GameBase*)item->getCollisionObject() != (GameBase*)mOwner && 
!item->isHidden() )
...
SimObjectPtr<SceneObject> safeThis(mOwner);
...
if(bool(safePtr))
safePtr->onCollision(mOwner,ptr->vector);
